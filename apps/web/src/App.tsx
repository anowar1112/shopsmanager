...
type Supplier = { id: string; name: string; phone: string; outstandingAmount: number };
...
function App() {
  ...
  const [screen, setScreen] = useState<'dashboard' | 'products' | 'inventory' | 'sales' | 'purchases' | 'expenses' | 'reports' | 'customers'>('dashboard');
...
function Dashboard({ user, onProducts, onInventory, onSales, onPurchases, onExpenses, onReports, onCustomers, onSignOut }: { user: User; onProducts: () => void; onInventory: () => void; onSales: () => void; onPurchases: () => void; onExpenses: () => void; onReports: () => void; onCustomers: () => void; onSignOut: () => void }) {
  ...
  return <main className="dashboard">
    <aside className="sidebar"><div className="brand-lockup"><div className="brand-mark small">R</div><span>Rahman Store</span></div><nav>{['Overview', 'Products', 'Inventory', 'Sales / POS', 'Purchases', 'Customers', 'Expenses', 'Reports'].map((item, index) => <button onClick={index === 1 ? onProducts : index === 2 ? onInventory : index === 3 ? onSales : index === 4 ? onPurchases : index === 5 ? onCustomers : index === 6 ? onExpenses : index === 7 ? onReports : undefined} className={index === 0 ? 'nav-item active' : 'nav-item'} key={item}><span>{['⌂', '□', '▦', '＋', '↗', '◎', '◌', '▥'][index]}</span>{item}</button>)}</nav><button className="nav-item sign-out" onClick={onSignOut}>↪ <span>Sign out</span></button></aside>
